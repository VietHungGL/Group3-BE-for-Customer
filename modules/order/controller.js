...
module.exports = {
  ...
  create: async function (req, res, next) {
    try {
      ...
      const { customerId, employeeId, orderDetails, paymentType, status, shippedDate, createdDate } = req.body;
      ...
      await asyncForEach(orderDetails, async (item, index, arr) => {
  ...
  updateStatus: async function (req, res, next) {
    try {
      ...
      let found = await Order.findOne({
        _id: id,
        $nor: [{ status: 'CANCELLED' }, { status: 'REJECTED' }, { status: 'COMPLETED' }]
      });
